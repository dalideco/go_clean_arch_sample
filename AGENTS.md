...
- Use `internal/log` (slog-backed) for **all** application logging. Import it as `"github.com/dali/go_clean_arch_sample/internal/log"`.
...
- Single package that owns both sides of the asynq + Redis boundary: the **producer** (`Client` + `EnqueueXxx`) and the **consumer** (`Server` + handlers). Both run in the same process — see `cmd/api/main.go`. Producer↔consumer payload contracts also live here — there is no separate `tasks/` package; co-locating with the producer + handler keeps a feature in one file.
...
- Shared infra in their own files: `client.go` (`Client` wrapping `*asynq.Client`, `New(cfg)`, `Close()`, `NewProducers(c) usecase.Producers`, `RedisOpt(cfg)`), `server.go` (`Server` wrapping `*asynq.Server`, `NewServer(cfg, repos)` registering every handler on the mux, non-blocking `Start()` + graceful `Shutdown()`; **also the queue registry + `DrainQueue`**), and `logger.go` (`NewAsynqLogger() asynq.Logger` — printf→`internal/log` bridge, same role gormLogger plays for GORM).
- **Queues are named per concern** (e.g. `QueueEmails = "emails"`), not one firehose. The `queues` map in `server.go` is the single source of truth for which queues exist and their weights — `NewServer` and `DrainQueue` both read it, so consumer + drainer never drift from producers. Each task enqueues into its queue with `asynq.Queue(QueueXxx)`; adding a queue = one entry in `queues` + the `asynq.Queue(...)` option at the enqueue site.
- **Testing async work**: don't run a `Server` and poll. `DrainQueue(ctx, cfg, repos)` is the asynq analog of `Oban.drain_queue` — it processes every pending task across all queues synchronously, inline in the test goroutine, via the same mux routing, then removes them. Tests never `Start` the server (the consumer stays "halted"), so jobs sit pending until drained deterministically. See `test/integration/create_user_test.go`.
- Handlers depend on the *narrowest* use-case-layer interface they actually need — usually `usecase.UserRepository` directly for read-only lookups, not the full `UserUseCase` (a read-only consumer has no business carrying write-side producers it never calls).
...
### `cmd/api/main.go`
- The process entrypoint, **not** the per-feature wiring root. Loads config, opens the DB + queue client, starts the **embedded asynq worker** (`queue.NewServer(cfg, repos).Start()`), assembles the `usecase.Deps` bundle (with `Repos: postgres.NewRepositories(db)` and `Producers: queue.NewProducers(client)`), hands it to `api.New(deps)`, runs the HTTP server with graceful shutdown.
- **The worker runs in-process, not as a separate binary.** This app never consumes tasks without producing them, so a standalone worker command would be indirection with no use case. On shutdown the HTTP server stops first (no new tasks enqueued), then `worker.Shutdown()` drains in-flight tasks.
- May import `internal/usecase` **only to name `usecase.Deps`** when building the wiring bundle. Must **not** import `internal/adapter/http/handler` or construct any `XxxUseCase` / `XxxHandler` — those live in the per-feature `router/<feature>.go` files. Adapter knowledge stays minimal: `postgres` (`startDbConnection` → `*gorm.DB`, then `postgres.NewRepositories`) and `queue` (`queue.New`, `queue.NewProducers`, `queue.NewServer`). `*gorm.DB` is named here and nowhere else outside `internal/adapter/repository/postgres/`; the same boundary holds for asynq inside `internal/queue/`.
...
9. **If the entity emits async events** (e.g. enqueues a task on create): declare the producer interface in `internal/usecase/foo.go` (e.g. `FooCreatedEnqueuer`), add the field to `usecase.Producers`, then create **one file** `internal/queue/foo_created.go` containing the payload struct + name constant + `(c *Client) EnqueueFooCreated(...)` method (satisfies the interface, enqueues with `asynq.Queue(QueueXxx)` for whichever queue fits — reuse an existing one in the `queues` registry or add a new entry) + `FooCreatedHandler.Handle(...)` consumer. Register the consumer with `mux.HandleFunc(queue.TypeFooCreated, queue.NewFooCreatedHandler(repos.Foos).Handle)` inside `newMux` (`internal/queue/server.go`) — the embedded worker and `DrainQueue` both pick it up, no `cmd/` change.
...
- **Lint:** `golangci-lint` (the Go credo equivalent) is the gatekeeper. Config in `.golangci.yml` enables `govet`, `staticcheck`, `errcheck`, `revive`, `unused`, `gocritic`, `gosec`, `gofmt`, `ineffassign`. Run via `mise run lint`.
- **Tests live next to source** as `_test.go` in the same package (so they can reach unexported helpers like `validateUser`). Use external `_test` packages only when an import cycle forces it.
- **Fakes are hand-written** and co-located with the test file. Don't reach for `mockery`/`gomock` until the interface surface is genuinely too large to maintain by hand. If two packages need the same fake, that's the signal to promote to `internal/<pkg>/testfakes/` — not before.
- **Per-entity coverage expectations:**
  - Domain factory: table tests over invariants + happy path + reconstitution-drift detection.
  - Use case: tests against fakes for every injected interface; verify both happy and policy-error paths (e.g. `ErrUserEmailTaken`), and the "log + continue" contract for best-effort side effects (enqueue, etc.).
  - Handler: `httptest` driving a real `*XxxUseCase` (with fake repo/enqueuer); assert envelope shape (`success`, `error`, `details`) and status per branch.
  - Repository: integration via `dockertest` — exercises CRUD + boundary translations (`gorm.ErrRecordNotFound` → `ErrXxxNotFound`, `pgconn 23505` → `ErrXxxEmailTaken`, reconstitution failure on drift). Gated by `testing.Short()` so unit runs stay fast.
- **CI** (`.github/workflows/ci.yml`) runs three parallel jobs: `lint`, `test-unit` (`-short`), `test-integration` (full). Independent — a lint failure doesn't mask a test signal.
- **Pre-push:** `mise run check` (lint + short tests). Integration runs in CI on PR.

## Build / verify

- `go build ./... && go vet ./...` should be clean before declaring work done.
- `mise run check` runs lint + unit tests; `mise run test` adds dockertest integration.
