...
- `handler/users.go` — depends on `*usecase.UserUseCase` directly (dependency direction allows it; the prior local `userUseCase` interface was a structural copy with no architectural seam, removed). Maps sentinels to 404 / 409, logs unexpected errors before 500.
...
- `internal/adapter/http/handler/users_test.go` — handler tests via `httptest`. Construct a real `*usecase.UserUseCase` with a fake `UserRepository` + fake `WelcomeEmailEnqueuer` (those interfaces are the legitimate dependency-inversion seams). Asserts envelope shape (`success`, `details`) and status codes for every branch. If this construction ever becomes painful, that's the signal to reintroduce a handler-side interface — not before.
